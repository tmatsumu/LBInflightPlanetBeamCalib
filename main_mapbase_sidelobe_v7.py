...
def noise_info_PhaseA1_LFT(nu_obs):
    if nu_obs == '40': NETarr = 17.3; det_num = 42.
    if nu_obs == '50': NETarr = 9.4; det_num = 56.
    if nu_obs == '60': NETarr = 9.7; det_num = 42.
    if nu_obs == '68': NETarr = 5.4; det_num = 170.
    if nu_obs == '78': NETarr = 4.9; det_num = 156.
    if nu_obs == '89': NETarr = 4.0; det_num = 170.
    if nu_obs == '100': NETarr = 4.8; det_num = 114.
    if nu_obs == '119': NETarr = 3.8; det_num = 114.
    if nu_obs == '140': NETarr = 3.6; det_num = 114.
    return NETarr*np.sqrt(det_num)

def noise_info_PhaseA1_HFT(nu_obs):
    if nu_obs == '100': NETarr = 4.6; det_num = 222.
    if nu_obs == '119': NETarr = 4.1; det_num = 148.
    if nu_obs == '140': NETarr = 2.9; det_num = 222.
    if nu_obs == '166': NETarr = 3.4; det_num = 148.
    if nu_obs == '195': NETarr = 2.8; det_num = 222.
    if nu_obs == '235': NETarr = 3.8; det_num = 148.
    if nu_obs == '280': NETarr = 4.4; det_num = 338.
    if nu_obs == '337': NETarr = 5.5; det_num = 338.
    if nu_obs == '402': NETarr = 9.4; det_num = 338.
    return NETarr*np.sqrt(det_num)
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

print '---'
print 'Dapt[mm]', sys.argv[0]
print 'Band[GHz]', sys.argv[1]
print 'src',sys.argv[2]
print 'dirout',sys.argv[3]
print 'option', sys.argv[4]
print '---'
...
obs_eff = 0.85*0.85
time_interval_month = 3.*obs_eff*3600.*24.*365.
time2spent_in_one_pixel = res_rad**2/(4.*pi)*time_interval_month
